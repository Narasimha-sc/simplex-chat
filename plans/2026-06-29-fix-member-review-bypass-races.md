...
There are **three** unlocked `runUpdateGroupProfile` callers. Introduce one
locked read-modify-write helper and route the pure-update callers through it; the
one caller that must conditionally throw keeps its own inline locked read.

```haskell
-- single critical section: lock → re-read gInfo → apply update fn → write
...
```
- `updateGroupProfileByName` (Commands.hs:3943) — resolve name→id, call helper.
  Covers `SetGroupMemberAdmissionReview`, `UpdateGroupNames`,
  `UpdateGroupDescription`, `SetGroupTimedMessages`, etc.
- `APIUpdateGroupProfile` (Commands.hs:3086) — `updateGroupProfileById user groupId (const p')`.
- `SetPublicGroupAccess` (Commands.hs:3097-3101) — **was missing from the first
  draft; found in adversarial review.** Keeps an inline `withGroupLock` + locked
  read because it must throw "not a public group" when `publicGroup` is `Nothing`
  (a pure `GroupProfile -> GroupProfile` update can't express that). The
  lock+read pattern therefore appears in exactly two places (the helper and this
  command) — minimal and justified.
...
`SetPublicGroupAccess`, now given its own inline group lock (it cannot use the
pure-update helper because it must conditionally throw "not a public group").
Alternatives
...
- `src/Simplex/Chat/Library/Commands.hs`:
  - `APIAcceptMember` — add `withGroupLock "acceptMember"` (Fix 1).
  - new `updateGroupProfileById` helper — group-locked read-modify-write (Fix 2).
  - `updateGroupProfileByName` — resolve id, call the helper (covers
    `UpdateGroupNames`/`UpdateGroupDescription`/`SetGroupMemberAdmissionReview`/
    `SetGroupTimedMessages`).
  - `APIUpdateGroupProfile` — `updateGroupProfileById user groupId (const p')`.
  - `SetPublicGroupAccess` — own inline `withGroupLock` + locked read (conditional
    throw, can't use the helper).
